...
LqgControlNode::LqgControlNode(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
  ...
  this->control_timer_ = this->create_wall_timer(std::chrono::milliseconds(timer_ms_),
                                                 std::bind(&LqgControlNode::controlCallback, this));

  this->registerStateSpaceIO();
  // Debug Publisher
  this->pubStateErrVect_ =
      this->create_publisher<std_msgs::msg::Float64MultiArray>("state_error", 10);
}

void LqgControlNode::registerStateSpaceIO() {
  // generic publishers
  this->pubCmdVect_ =
      this->create_publisher<std_msgs::msg::Float64MultiArray>("command_vector", 10);
  // generic subscribers
  this->subMeasVect_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "measurements", *qos_,
      std::bind(&LqgControlNode::updateMeasurement, this, std::placeholders::_1));
  this->subCovMat_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "measurements_cov", *qos_,
      std::bind(&LqgControlNode::updateMeasurementCov, this, std::placeholders::_1));
  this->subDesVect_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "desired_states", *qos_,
      std::bind(&LqgControlNode::updateDesiredState, this, std::placeholders::_1));
}

rcl_interfaces::msg::SetParametersResult LqgControlNode::paramUpdateCallback(
  ...
  // cycle through the list of set parameters and update temporarily-stored
  // parameter table.
  ...
  // at the end of each cycle, we need to check if the set of
  // temporarily-stored parameters meet format consistency. If the format
  // consistency is met, we automatically update the controller.
